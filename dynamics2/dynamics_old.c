  ...
  char *file_name;
{
  ...
  for (i=0;i<num_adorbs;i++){
    ...
    ao.re[details->adsorbate_orbs[i]] = MATRIX(adsorb_ham,k,i);
  }
  ...
  for (i=0;i<num_orbs;i++){
    for (j=0;j<num_orbs;j++){
      mo.re[i] += TRANSFORM(eigenset,i,j)*ao.re[j];
    }
    norm += mo.re[i]*mo.re[i];
  }        
  ...
  real timestep;
{
  ...
  for (i=0;i<psi.dim;i++){
    save_psi = psi.re[i];
    psi.re[i] = psi.re[i]*cos(ENERGY(eigen,i)*timestep*(1/hbar))+
	        psi.im[i]*sin(ENERGY(eigen,i)*timestep*(1/hbar));
    psi.im[i] = psi.im[i]*cos(ENERGY(eigen,i)*timestep*(1/hbar))-
	        save_psi*sin(ENERGY(eigen,i)*timestep*(1/hbar));
  }
  ...
  wavepacket_type mo,save_mo;
{
  ...
  isurv=0.0;
  for (i=0;i<mo.dim;i++){
    surv += save_mo.re[i]*mo.re[i];
    isurv += save_mo.re[i]*mo.im[i];
  }
  return pow(surv*surv+isurv*isurv,0.5);
}
