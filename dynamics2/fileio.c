  ...
  FILE *the_file;
{
  ...
  /* Variables for Dynamics - Robert Snoeberger*/
  char dash;
  int start,stop;
  /* End Robert Snoeberger */
...
/* Wendu Ding */
  details->VAalign = VA_align;
  details->VBalign = VB_align;
  details->KAalign = KA_align;
  details->KBalign = KB_align;
  details->Aorb = A_orb_num;
/* Wendu Ding Done */
 ...
 evaluated.\n");
	...
	fprintf(status_file,"K = %lf\n",details->the_const);
      ...
      /* Wendu Ding, for Efermi alignment */
      else if(strstr(instring,"EFALIGNMENT1")){
        fprintf(status_file,"Reading in new values for the constant K and shifting.\n");
        if( sscanf(instring,"%s %lf %lf",string1,
                   &(details->VAalign),&(details->KAalign)) != 3 ){
          skipcomments(infile,instring,FATAL);
          sscanf(instring,"%lf %lf",&(details->VAalign),&(details->KAalign));
        }
        fprintf(status_file,"VA = %lf\nKA = %lf\n",
                details->VAalign,details->KAalign);
      }
      else if(strstr(instring,"EFALIGNMENT2")){
        fprintf(status_file,"Reading in new values for the constant K and shifting.\n");
        if( sscanf(instring,"%s %lf %lf %lf %lf %d",string1,
                   &(details->VAalign),&(details->VBalign),
                   &(details->KAalign),&(details->KBalign),
                   &(details->Aorb)) != 6 ){
          skipcomments(infile,instring,FATAL);
          sscanf(instring,"%lf %lf %lf %lf %d",&(details->VAalign),&(details->VBalign),
                 &(details->KAalign),&(details->KBalign),&(details->Aorb));
        }
        fprintf(status_file,"VA = %lf\nVB = %lf\nKA = %lf\nKB = %lf\nAorb = %d\n",
                details->VAalign,details->VBalign,
                details->KAalign,details->KBalign,details->Aorb);
...
calculation.\n");
	...
	for(i=0;i<details->num_proj_DOS;i++){
	  ...
	  ****/
      ...
      /*--------------------------------------------------------*/ 
      else if( strstr(instring,"DYNAMICS") ){
	//printf("Asking for an electron dynamics simulation.\n");
	/* Set printing of hamiltonian to true - Saves hamiltonian matrix - VERY IMPORTANT!!*/
	details->hamil_PRT = 1;
        /* Set dynamics to true */
	details->dynamics = 1;
	/* Get timestep and time length*/
	skipcomments(infile,instring,FATAL);
	sscanf(instring,"%lf %lf",&(details->timestep),&(details->time_length));
	//printf("Run dyn for %f with %f step\n",details->time_length,details->timestep);
      }

      /*--------------------------------------------------------*/
      else if( strstr(instring,"ADSORB") ){
	//printf("An adsorbate has been found as inital wavepacket in electron dynamics.\n");
	/* Get number of atoms in adsorbate and the inital orbitals number */
	skipcomments(infile,instring,FATAL);
        /* Initialize initial orbital arrays */
        details->initial_len = 10;
        for (i=0;i<details->initial_len;i++){
          details->initial_orbital[i] = 1;
          details->initial_super[i] = 0.0;
        }
        token = strtok(instring," ,\n");
        sscanf(token,"%d",&(details->adsorbate_num_atoms));
        i=0;
        j=0;
        token = strtok(NULL," ,\n");
        while (token != NULL && i < 10){
          sscanf(token,"%d",&(details->initial_orbital[i]));
          token = strtok(NULL," ,\n");
          sscanf(token,"%lf",&(details->initial_super[i]));
          token = strtok(NULL," ,\n");
          i += 1; 
        }
        //printf("Checking Adsorbate Input Parsing!\n");
        //printf("Number of Adsorbate Atoms: %d\n",details->adsorbate_num_atoms);
        //for (i=0;i<details->initial_len;i++){
        //  printf("%d %f\n",details->initial_orbital[i],details->initial_super[i]);
        //}
        

	//sscanf(instring,"%d %d",&(details->adsorbate_num_atoms),&(details->initial_orbital));
	
	/* Alocate space for indicies of atoms in adsorbate*/
	details->adsorbate = (int *)calloc(details->adsorbate_num_atoms,sizeof(int));
	/* Get indicies of atoms in adsorbate, use strtok to seperate at commas*/
        skipcomments(infile,instring,FATAL);
	safe_strcpy(tempstring,(char *)strtok(instring,",\n"));
	sscanf(instring,"%d",&(details->adsorbate[0]));
	details->adsorbate[0]--;
	i = 1;
	done = 0;
	while(!done){
	  ...
	  if( !tempstring || tempstring[0] == 0){
	    ...
	    if (strstr(tempstring,"-")){
	      sscanf(tempstring,"%d%c%d",&start,&dash,&stop);
	      for (j=start;j<=stop;j++){
		details->adsorbate[i] = j-1;
		i++;
	      }
	    } else{
	      sscanf(tempstring,"%d",&(details->adsorbate[i]));
	      details->adsorbate[i]--;
	      i++; 
	    }
	  }
	...
	//}
      ...
      /*----------------------------------------------------------*/ 
      else if( strstr(instring,"ABSORB") ){
	/* Set absorbing potential to true */
        details->absorbing = 1;
	/* get number of atoms with abosrbing potential */
	skipcomments(infile,instring,FATAL);
	sscanf(instring,"%d%lf",&(details->absorbing_dim),&(details->absorbing_value));
	/* Allocate Space for indices of atoms and absorbing potential value */
	details->absorbing_orbs = (int *)calloc(details->absorbing_dim,sizeof(int));
	/* Get indicies and values */
        skipcomments(infile,instring,FATAL);
	safe_strcpy(tempstring,(char *)strtok(instring,",\n"));
	sscanf(instring,"%d",&(details->absorbing_orbs[0]));
	details->absorbing_orbs[0]--;
	i = 1;
	done = 0;
	while(!done){
	  ...
	  if( !tempstring || tempstring[0] == 0){
	    ...
	    sscanf(tempstring,"%d",&(details->absorbing_orbs[i]));
	    details->absorbing_orbs[i]--;
	    i++; 
	  }
        }	  
      }
      /**********************************************************/
      else if (strstr(instring,"COUPLING") ){
        /* Set electronic coupling to true */
        details->coupling = 1;
	/* Set printing of hamiltonian to true - Saves hamiltonian matrix - VERY IMPORTANT!!*/
	details->hamil_PRT = 1;
        /* get state of interest */
        skipcomments(infile,instring,FATAL);
        sscanf(instring,"%d  %lf",&(details->coupling_state),&(details->coupling_width));
      }

      /**********************************************************/
      else if (strstr(instring,"PRINT SYSTEM") ){
	/* Set print_system to true */
	details->print_system = 1;
	/* Set printing of hamiltonian to true - Saves hamiltonian matrix - VERY IMPORTANT!!*/
	details->hamil_PRT = 1;
      }
      ...
      /*-----------------------------------------------------------*/
      else if( strstr(instring,"OCCUPATION") ){
        /* Set occupation to true */
	details->occupation = 1;
	/* get number of atoms */
	skipcomments(infile,instring,FATAL);
	sscanf(instring,"%d",&(details->occ_dim));
	/* Allocate Space for indices of atoms and absorbing potential value */
	details->occ_atoms = (int *)calloc(details->occ_dim,sizeof(int));
	/* Get indicies of atoms in occupation, use strtok to seperate at commas*/
        skipcomments(infile,instring,FATAL);
	safe_strcpy(tempstring,(char *)strtok(instring,",\n"));
	sscanf(instring,"%d",&(details->occ_atoms[0]));
	details->occ_atoms[0]--;
	i = 1;
	done = 0;
	while(!done){
	  ...
	  if( !tempstring || tempstring[0] == 0){
	    ...
	    if (strstr(tempstring,"-")){
	      sscanf(tempstring,"%d%c%d",&start,&dash,&stop);
	      for (j=start;j<=stop;j++){
		details->occ_atoms[i] = j-1;
		i++;
	      }
	    } else{
	      sscanf(tempstring,"%d",&(details->occ_atoms[i]));
	      details->occ_atoms[i]--;
	      i++; 
	    }
	  }
      ...
      /*--------------------------------------------------------*/
      else if( strstr(instring,"PSEUDO") ){
	//printf("Pseudoboundaries will be used in electron dynamics.\n");
	/* Set pseudoboundy to true */
	details->pseudoboundary = 1;
	/* get x,y and z lattice lengths */
	skipcomments(infile,instring,FATAL);
	sscanf(instring,"%lf %lf %lf",&(details->pseudo_x),&(details->pseudo_y),&(details->pseudo_z));
	//printf("Pseudovectors %f %f %f\n",details->pseudo_x,details->pseudo_y,details->pseudo_z);
      }

      /*--------------------------------------------------------*/
      else if( strstr(instring,"CUBE") ){
	/* Set cube to true */
        details->cube = 1;
	/* get step size */
	//skipcomments(infile,instring,FATAL);
	//sscanf(instring,"%lf",&(details->cube_step));
	/* get xmin xmax */
	//skipcomments(infile,instring,FATAL);
	//sscanf(instring,"%lf %lf",&(details->cube_region[0]),&(details->cube_region[1]));
	//printf("xmin = %f, xmax = %f\n",details->cube_region[0],details->cube_region[1]);
	/* get ymin xmax */
	//skipcomments(infile,instring,FATAL);
	//sscanf(instring,"%lf %lf",&(details->cube_region[2]),&(details->cube_region[3]));
	//printf("ymin = %f, ymax = %f\n",details->cube_region[2],details->cube_region[3]);
	/* get zmin xmax */
	//skipcomments(infile,instring,FATAL);
	//sscanf(instring,"%lf %lf",&(details->cube_region[4]),&(details->cube_region[5]));
	//printf("zmin = %f, zmax = %f\n",details->cube_region[4],details->cube_region[5]);
      }

      /**********************************************************/

      else if( strstr(instring,"RESTART") ){
        /* Set restart to 1 */
        details->restart = 1;
        /* get restart filename */
        skipcomments(infile,instring,FATAL);
        sscanf(instring,"%s",&(details->restart_name[0]));
        strcpy(details->restart_name,instring);
        i = strlen(details->restart_name);
        details->restart_name[i-1] = '\0';
        printf("Using restart file %s\n",&(details->restart_name));
      }
      /*--------------------------------------------------------*/ 
