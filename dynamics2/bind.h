...
#define VA_align 0.000000
#define VB_align 0.000000
#define KA_align 1.750000
#define KB_align 1.750000
#define A_orb_num 100000
...
#define TRANSFORM(set,row,column) set.vectR[row*set.dim + column] /* by Robert for Dynamics */
#define ENERGY(set,which) set.val[which]
...
#define MATRIX(matrix,i,j) matrix.mat[i*matrix.dim + j] /* by Robert for Dynamics */
...
/* Used to store the wavepacket in dynamics: by Robert */
typedef struct{
  int dim;
  real *re;
  real *im;
...
typedef struct{
  ...
  *****/
  ...
  int *adsorbate;
  int *adsorbate_orbs;
  int adsorbate_num_atoms;
  int adsorbate_num_orbs;
  int initial_len;
  int initial_orbital[10];
  double initial_super[10];
  /* Absorbing Potential */
  BOOLEAN absorbing;
  int absorbing_dim;
  int *absorbing_orbs;
  real absorbing_value;
  real *absorbing_pot;
  /* Occupation */
  BOOLEAN occupation;
  int occ_dim;
