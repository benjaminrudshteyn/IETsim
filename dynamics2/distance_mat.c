  ...
  ********/
  ...
  for(i=0;i<num_atoms;i++){
    for(j=0;j<i;j++){
      ...
      /* Robert Snoeberger
       * Pseudo Boundaries for Electron Dynamics
       * */
      if (details->pseudoboundary){
	/* All these if statements are probably slow computationally
	 *   Is there a better way? */
        if (temp.x >= 0.5*details->pseudo_x){
          temp.x = details->pseudo_x - temp.x;
	}
	/* The temp distance can be negative */
	else if (temp.x <= -0.5*details->pseudo_x){
          temp.x = -details->pseudo_x - temp.x; // add on the temp distance
	}
	if (temp.y >= 0.5*details->pseudo_y){
          temp.y = details->pseudo_y - temp.y;
	}
	else if (temp.y <= -0.5*details->pseudo_y){
          temp.y = -details->pseudo_y - temp.y;
	}
	if (temp.z >= 0.5*details->pseudo_z){
          temp.z = details->pseudo_z - temp.z;
	}     
	else if (temp.z <= -0.5*details->pseudo_z){
          temp.z = -details->pseudo_z - temp.z;
	}
      }
      /*
       * End Pseudo Boundaries
       * */
