		    ...
		    int *orbital_lookup_table)
{
  ...
  for(i=0;i<cell->num_atoms;i++){
	...
	if(j_tab >= 0){
	  ...
	  /* Robert Snoeberger
	   * Applying pseudoboundaries
	   * */
          if (details->pseudoboundary){
	    if (dist_vect.x >= 0.5*details->pseudo_x){
              dist_vect.x = -(details->pseudo_x - dist_vect.x);
	    }
	    /* Repeating for negative distances */
	    else if (dist_vect.x <= -0.5*details->pseudo_x){
              dist_vect.x = -(-details->pseudo_x - dist_vect.x);
	    }
	    if (dist_vect.y >= 0.5*details->pseudo_y){
              dist_vect.y = -(details->pseudo_y - dist_vect.y);
	    }
	    else if (dist_vect.y <= -0.5*details->pseudo_y){
              dist_vect.y = -(-details->pseudo_y - dist_vect.y);
	    }
	    if (dist_vect.z >= 0.5*details->pseudo_z){
              dist_vect.z = -(details->pseudo_z - dist_vect.z);
	    }
	    else if (dist_vect.z <= -0.5*details->pseudo_z){
              dist_vect.z = -(-details->pseudo_z - dist_vect.z);
	    }
	  }
