  ...
  int *orbital_lookup_table;
{
  int i,j;
  int orb_tab1;
  real Kialign,Kjalign,Vialign,Vjalign;
  atom_type *atom_ptr1;
...
/* Wendu Ding */
    Vialign=details->VAalign;
    for(i=0;i<num_orbs;i++){
    if(i>=details->Aorb){
      Vialign=details->VBalign;}
    hamil.mat[i*num_orbs+i] = hamil.mat[i*num_orbs+i]+Vialign;
  }
/* Wendu Ding Done */
}    
  ...
  for(i=0;i<cell->num_atoms;i++){
	...
	diagonal_elements[orb_tab1+BEGIN_S] =
	  hamil.mat[(orb_tab1+BEGIN_S)*num_orbs+orb_tab1+BEGIN_S] =
	    atom_ptr1->coul_s;
//printf("%d,%d,%d,%d\n",orb_tab1,BEGIN_S,num_orbs,(orb_tab1+BEGIN_S)*num_orbs+orb_tab1+BEGIN_S);
      }
      if( atom_ptr1->np != 0 ){
	for(j=BEGIN_P;j<=END_P;j++){
...
*/
/*  Wendu Ding   */
        Kialign=details->KAalign;
        Vialign=details->VAalign;
        Kjalign=details->KAalign;
        Vjalign=details->VAalign;
        if(i>=details->Aorb){ 
          Kialign=details->KBalign;
          Vialign=details->VBalign;}
        if(j>=details->Aorb){
          Kjalign=details->KBalign;
          Vjalign=details->VBalign;}
          
          hamil.mat[j*num_orbs+i] = .5*(Kialign*diagonal_elements[i]+Vialign +
                                        Kjalign*diagonal_elements[j]+Vjalign);
...
/* Wendu Ding */
    ...
    if(i>=details->Aorb){
      Vialign=details->VBalign;}
    diagonal_elements[i] = diagonal_elements[i]+Vialign;
  }
/* Wendu Ding Done */
