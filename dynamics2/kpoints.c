  ...
  int *orbital_lookup_table;
{
  ...
  int num_orbs2;
#endif
  time_t time0,time1; //For Robert Snoeberger - Timing Diagonalization
...
#pragma pfor iterate (i=0;num_KPOINTS;1)
  ...
  for(i=0;i<num_KPOINTS;i++){
	...
	*******/
    ...
    if( details->hamil_PRT ){ /* && !details->dynamics && !details->coupling ){*/
      fprintf(output_file,
	      ";\t\t --- Hamiltonian ");
      if( details->Execution_Mode == MOLECULAR ){
	...
	********/
      ...
      cboris(&(num_orbs),&(num_orbs),hamilK.mat,work3,eigenset.vectI,eigenset.val,work1,
	     work2,&diag_error);
      time1 = time(NULL);
      fprintf(output_file,"\nDiagonalized in %6.2f min.\n\n",difftime(time1,time0)/60);
