...
//  argc = ccommand(&argv);
  ...
  ***********/
  ...
  /* Robert Snoeberger Stuff */

 /* 
  *  Print Diagonalized System for restarting
  */
 if (details->print_system){
    print_system(unit_cell,details,Overlap_R,Hamil_R,Overlap_K,
			   Hamil_K,cmplx_hamil,cmplx_overlap,
			   eigenset,work1,work2,work3,cmplx_work,
			   &properties,
			   avg_prop_info,num_orbs,orbital_lookup_table,
 ...
 *  */
 if (details->coupling==1){
    printf("Calculating Electronic Coupling to State %d\n",
           details->coupling_state);
    ecoupling(unit_cell,details,Overlap_R,Hamil_R,Overlap_K,
			   Hamil_K,cmplx_hamil,cmplx_overlap,
			   eigenset,work1,work2,work3,cmplx_work,
			   &properties,
			   avg_prop_info,num_orbs,orbital_lookup_table,
  ...
  /* End electronic coupling calculation */

  /* Robert Snoeberger 
   * Run Electron Dynamics 
   * */

  /* Filename for electron dynamics data */
  strcpy(file_name,argv[1]);
  strcat(file_name,".edyn");
  
  
  if (details->dynamics){
    printf("Starting Dynamics\n");
    dynamics(unit_cell,details,Overlap_R,Hamil_R,Overlap_K,
			   Hamil_K,cmplx_hamil,cmplx_overlap,
			   eigenset,work1,work2,work3,cmplx_work,
			   &properties,
			   avg_prop_info,num_orbs,orbital_lookup_table,file_name);
  }	  

  /* End Electron Dynamics*/
